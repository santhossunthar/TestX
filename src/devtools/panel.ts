...
let startProxyAutomatedTestingBtn = document.querySelector<HTMLButtonElement>("#startProxyAutomatedTestingBtn");
...
let securityResultModal = document.querySelector<HTMLDivElement>("#securityResultModal");
let securityResultModalTitle = document.querySelector<HTMLHeadingElement>("#securityResultModalTitle");
let securityResultModalEndpoint = document.querySelector<HTMLDivElement>("#securityResultModalEndpoint");
let securityResultModalSummary = document.querySelector<HTMLDivElement>("#securityResultModalSummary");
let securityResultModalOverview = document.querySelector<HTMLDivElement>("#securityResultModalOverview");
let securityResultModalFindings = document.querySelector<HTMLDivElement>("#securityResultModalFindings");
let securityResultCloseBtn = document.querySelector<HTMLButtonElement>("#securityResultCloseBtn");
...
const ensureSecurityResultModalMarkup = () => {
  if (
    securityResultModal &&
    securityResultModalTitle &&
    securityResultModalEndpoint &&
    securityResultModalSummary &&
    securityResultModalOverview &&
    securityResultModalFindings &&
    securityResultCloseBtn
  ) {
    return;
  }
  ...
  wrapper.innerHTML = `
    <div
      id="securityResultModal"
      class="modal hidden"
      role="dialog"
      aria-modal="true"
      aria-labelledby="securityResultModalTitle"
    >
      <div class="modal-card security-modal-card">
        <h3 id="securityResultModalTitle">Endpoint Security Analysis</h3>
        <div id="securityResultModalEndpoint" class="helper-text"></div>
        <div id="securityResultModalSummary" class="helper-text"></div>
        <div id="securityResultModalOverview" class="helper-text"></div>
        <div id="securityResultModalFindings" class="endpoint-list"></div>
        <div class="inline">
          <button id="securityResultCloseBtn" type="button">Close</button>
        </div>
      </div>
    </div>
  `;
  ...
  securityResultModal = document.querySelector<HTMLDivElement>("#securityResultModal");
  securityResultModalTitle = document.querySelector<HTMLHeadingElement>("#securityResultModalTitle");
  securityResultModalEndpoint = document.querySelector<HTMLDivElement>("#securityResultModalEndpoint");
  securityResultModalSummary = document.querySelector<HTMLDivElement>("#securityResultModalSummary");
  securityResultModalOverview = document.querySelector<HTMLDivElement>("#securityResultModalOverview");
  securityResultModalFindings = document.querySelector<HTMLDivElement>("#securityResultModalFindings");
  securityResultCloseBtn = document.querySelector<HTMLButtonElement>("#securityResultCloseBtn");
};
...
let activeRepeaterResponseTab: "raw" | "headers" | "body" = "raw";
...
const ensureRepeaterResponseTabMarkup = () => {
  if (document.querySelector("#repeaterResponseRawTabBtn")) return;
  const existingRaw = repeaterResponseSection.querySelector<HTMLPreElement>("#repeaterResponseBody");
  const existingText = existingRaw?.textContent ?? "";
  repeaterResponseSection.innerHTML = `
    <div class="repeater-row-head">
      <h4>Response</h4>
      <div class="repeater-request-tabs">
        <button id="repeaterResponseRawTabBtn" type="button" class="repeater-request-tab is-active">Raw</button>
        <button id="repeaterResponseHeadersTabBtn" type="button" class="repeater-request-tab">Headers</button>
        <button id="repeaterResponseBodyTabBtn" type="button" class="repeater-request-tab">Body</button>
      </div>
    </div>
    <div id="repeaterResponseRawPane" class="repeater-response-pane">
      <pre id="repeaterResponseBody"></pre>
    </div>
    <div id="repeaterResponseHeadersPane" class="repeater-response-pane hidden">
      <pre id="repeaterResponseHeadersOutput"></pre>
    </div>
    <div id="repeaterResponseBodyPane" class="repeater-response-pane hidden">
      <pre id="repeaterResponseOnlyBody"></pre>
    </div>
  `;
  const rawPre = repeaterResponseSection.querySelector<HTMLPreElement>("#repeaterResponseBody");
  if (rawPre) {
    rawPre.textContent = existingText;
  }
};
...
const hideSecurityResultModal = () => {
  securityResultModal.classList.add("hidden");
  securityResultModalFindings.innerHTML = "";
};

const renderSecurityResultModal = (endpoint: DiscoveredEndpoint, result: EndpointTestResult) => {
  const statusText = result.error
    ? `Failed - ${result.error}`
    : `${result.status ?? "-"} in ${result.elapsedMs?.toFixed(0) ?? "-"}ms`;
  const findings = result.securityFindings ?? [];
  securityResultModalTitle.textContent = "Endpoint Security Analysis";
  securityResultModalEndpoint.textContent = `${endpoint.method} ${endpoint.url}`;
  securityResultModalSummary.textContent = `Result: ${statusText}`;
  securityResultModalOverview.textContent =
    result.securityOverview || summarizeSecurityFindings(findings);
  if (findings.length === 0) {
    securityResultModalFindings.innerHTML = `<div class="helper-text">No specific security findings were detected.</div>`;
  } else {
    securityResultModalFindings.innerHTML = findings
      .map(
        (finding) => `<div class="security-finding">
      <div class="inline">
        <strong>${escapeHtml(finding.title)}</strong>
        <span class="security-severity ${finding.severity}">${finding.severity}</span>
      </div>
      <div class="helper-text">Evidence: ${escapeHtml(finding.evidence || "-")}</div>
      <div class="helper-text">Recommendation: ${escapeHtml(finding.recommendation)}</div>
    </div>`
      )
      .join("");
  }
  securityResultModal.classList.remove("hidden");
};
    ...
    .map(
      ...
      ${endpoints
        .map((endpoint) => {
          ...
          const findingCount = result.securityFindings?.length ?? 0;
          const highCount = result.securityFindings?.filter((item) => item.severity === "high").length ?? 0;
          const statusLine = result.error
            ? `Failed - ${result.error}`
            : `${result.status ?? "-"} in ${result.elapsedMs?.toFixed(0) ?? "-"}ms`;
          return `<div class="endpoint-item is-clickable" data-endpoint-id="${endpoint.id}">
        <div class="endpoint-item-head">
          <span class="endpoint-method">${endpoint.method}</span>
          <span class="endpoint-url">${endpoint.url}</span>
        </div>
        ...
        <div class="helper-text">Security: ${findingCount} finding(s), ${highCount} high severity</div>
        <div class="helper-text">Click to view full security analysis.</div>
      </div>`;
        })
        .join("")}
    </div>`
...
const requestEndpointTestAnalysis = async (
  ...
  responseText: string
): Promise<EndpointSecurityAnalysis> => {
  const localFindings = performLocalSecurityAssessment(endpoint, responseStatus, responseHeaders, responseText);
  let aiSummary = "";
  let aiOverview = "";
  ...
  try {
    const parsed = (await callAiForJson(
      aiSettings,
      aiApiKeyMemory,
      "You are an API security analyzer. Return only JSON object: {\"summary\":string,\"overview\":string,\"findings\":[{\"title\":string,\"severity\":\"low|medium|high\",\"evidence\":string,\"recommendation\":string}]}. Focus on missing security headers, information disclosure, insecure cookie/CORS settings, and remediation.",
      {
        endpoint: { method: endpoint.method, url: endpoint.url, group: endpoint.group },
        request: { body: requestBody ? requestBody.slice(0, 1800) : "" },
        response: {
    ...
    if (typeof parsed.overview === "string" && parsed.overview.trim()) aiOverview = parsed.overview.trim();
    if (Array.isArray(parsed.findings)) {
      aiFindings = parsed.findings
        .map((item) => {
          ...
          const severityRaw = typeof item.severity === "string" ? item.severity.toLowerCase() : "low";
          ...
          return { title, severity, evidence, recommendation };
        })
        .filter((item): item is SecurityFinding => Boolean(item));
    }
  } catch {}
  const securityFindings = mergeSecurityFindings(localFindings, aiFindings);
  const securityOverview = aiOverview || summarizeSecurityFindings(securityFindings);
  const summary =
    aiSummary ||
    (responseStatus >= 500
      ? `Server-side failure (HTTP ${responseStatus}) with ${securityFindings.length} potential security signal(s).`
      : responseStatus >= 400
        ? `Client/auth issue (HTTP ${responseStatus}) with ${securityFindings.length} potential security signal(s).`
        : `Endpoint responded (HTTP ${responseStatus}); ${securityFindings.length} potential security signal(s) detected.`);
  return { summary, securityOverview, securityFindings };
};
...
const runAutomatedEndpointTesting = async (endpointIds: string[]) => {
  ...
  for (const endpointId of endpointIds) {
    ...
    try {
      ...
      const analysis = await requestEndpointTestAnalysis(endpoint, requestBody, res.status, headersObj, text);
      endpointTestResults.set(endpoint.id, {
        endpointId: endpoint.id,
        status: res.status,
        elapsedMs: elapsed,
        summary: analysis.summary,
        securityOverview: analysis.securityOverview,
        securityFindings: analysis.securityFindings,
        requestHeaders: parsedRequestHeaders,
        requestBody,
        responseHeaders: headersObj,
        responseBodySnippet: text.slice(0, 6000)
      });
    } catch (error) {
      const message = error instanceof Error ? error.message : String(error);
      endpointTestResults.set(endpoint.id, {
...
const getProxyDrivenEndpointIdsForTesting = (): string[] => {
  if (discoveredEndpoints.length === 0) return [];
  if (proxyTrafficEntries.length === 0) {
    return discoveredEndpoints.map((endpoint) => endpoint.id);
  }

  const endpointIdByKey = new Map<string, string>();
  discoveredEndpoints.forEach((endpoint) => {
    endpointIdByKey.set(`${endpoint.method} ${sanitizeDiscoveredUrl(endpoint.url)}`, endpoint.id);
  });

  const matched = new Set<string>();
  proxyTrafficEntries.forEach((entry) => {
    if (entry.type !== "http") return;
    const methodCandidate = entry.method.toUpperCase() as HttpMethod;
    const method = METHOD_SET.has(methodCandidate) ? methodCandidate : "GET";
    const key = `${method} ${sanitizeDiscoveredUrl(entry.url)}`;
    const id = endpointIdByKey.get(key);
...
const ensureProxyAutomatedTestingButton = (): HTMLButtonElement | null => {
  if (startProxyAutomatedTestingBtn) return startProxyAutomatedTestingBtn;
  const proxyTabbar = document.querySelector<HTMLDivElement>(".proxy-tabbar");
  if (!proxyTabbar) return null;
  const button = document.createElement("button");
  button.id = "startProxyAutomatedTestingBtn";
  button.type = "button";
  button.textContent = "Start Automated Testing";
  proxyTabbar.insertBefore(button, clearProxyTrafficBtn);
  startProxyAutomatedTestingBtn = button;
  return button;
};
...
const renderRepeaterResponseTab = () => {
  const responseRawTabBtn = document.querySelector<HTMLButtonElement>("#repeaterResponseRawTabBtn");
  const responseHeadersTabBtn = document.querySelector<HTMLButtonElement>("#repeaterResponseHeadersTabBtn");
  const responseBodyTabBtn = document.querySelector<HTMLButtonElement>("#repeaterResponseBodyTabBtn");
  const responseRawPane = document.querySelector<HTMLDivElement>("#repeaterResponseRawPane");
  const responseHeadersPane = document.querySelector<HTMLDivElement>("#repeaterResponseHeadersPane");
  const responseBodyPane = document.querySelector<HTMLDivElement>("#repeaterResponseBodyPane");
  if (
    !responseRawTabBtn ||
    !responseHeadersTabBtn ||
    !responseBodyTabBtn ||
    !responseRawPane ||
    !responseHeadersPane ||
    !responseBodyPane
  ...
  const rawActive = activeRepeaterResponseTab === "raw";
  const headersActive = activeRepeaterResponseTab === "headers";
  const bodyActive = activeRepeaterResponseTab === "body";
  responseRawTabBtn.classList.toggle("is-active", rawActive);
  responseHeadersTabBtn.classList.toggle("is-active", headersActive);
  responseBodyTabBtn.classList.toggle("is-active", bodyActive);
  responseRawPane.classList.toggle("hidden", !rawActive);
  responseHeadersPane.classList.toggle("hidden", !headersActive);
  responseBodyPane.classList.toggle("hidden", !bodyActive);
};

const setRepeaterResponseContent = (raw: string, headers: string, body: string) => {
  const responseRaw = document.querySelector<HTMLPreElement>("#repeaterResponseBody");
  const responseHeaders = document.querySelector<HTMLPreElement>("#repeaterResponseHeadersOutput");
  const responseBodyOnly = document.querySelector<HTMLPreElement>("#repeaterResponseOnlyBody");
  if (responseRaw) responseRaw.textContent = raw;
  if (responseHeaders) responseHeaders.textContent = headers;
  if (responseBodyOnly) responseBodyOnly.textContent = body;
};

const buildActiveRepeaterResponseParts = (
  entry: RepeaterEntry | undefined
): { raw: string; headers: string; body: string } => {
  if (!entry) return { raw: "", headers: "", body: "" };
  const flow = entry.flowSteps ?? [];
  if (flow.length > 0) {
    const index = Math.max(0, Math.min(entry.activeFlowIndex ?? flow.length - 1, flow.length - 1));
    const step = flow[index];
    const statusLine =
      step.responseStatus > 0
        ? `${step.responseStatus} ${step.responseStatusText}`.trim()
        : step.responseStatusText.trim();
    const headersOnly = [statusLine, step.responseHeaders.trim()].filter(Boolean).join("\n");
    const bodyOnly = step.responseBody ? formatJsonIfPossible(step.responseBody) : "";
    const rawParts: string[] = [];
    if (headersOnly) rawParts.push(headersOnly);
    if (bodyOnly) rawParts.push("", bodyOnly);
    const raw = rawParts.join("\n").trim();
    return { raw, headers: headersOnly, body: bodyOnly };
  }
  ...
  if (splitIndex >= 0) {
    ...
    return { raw, headers: head, body: formatJsonIfPossible(body) };
  }

  return { raw, headers: "", body: formatJsonIfPossible(normalized) };
};
...
interface EndpointSecurityAnalysis {
  summary: string;
  securityOverview: string;
  securityFindings: SecurityFinding[];
}

const pushSecurityFinding = (
  ...
  findings.push({ title, severity, evidence, recommendation });
};

const getHeaderValueCaseInsensitive = (headers: Record<string, string>, key: string): string => {
  const lowered = key.toLowerCase();
  const match = Object.entries(headers).find(([name]) => name.toLowerCase() === lowered);
  return match?.[1] ?? "";
};

const summarizeSecurityFindings = (findings: SecurityFinding[]): string => {
  if (findings.length === 0) return "No obvious security weakness was detected from this response.";
  const high = findings.filter((item) => item.severity === "high").length;
  const medium = findings.filter((item) => item.severity === "medium").length;
  const low = findings.filter((item) => item.severity === "low").length;
  return `Detected ${findings.length} potential issue(s): ${high} high, ${medium} medium, ${low} low.`;
};

const performLocalSecurityAssessment = (
  endpoint: DiscoveredEndpoint,
  responseStatus: number,
  responseHeaders: Record<string, string>,
  responseText: string
  ...
  const urlIsHttps = endpoint.url.startsWith("https://");
  const hsts = getHeaderValueCaseInsensitive(responseHeaders, "strict-transport-security");
  const csp = getHeaderValueCaseInsensitive(responseHeaders, "content-security-policy");
  const xcto = getHeaderValueCaseInsensitive(responseHeaders, "x-content-type-options");
  const xfo = getHeaderValueCaseInsensitive(responseHeaders, "x-frame-options");
  const server = getHeaderValueCaseInsensitive(responseHeaders, "server");
  const xPoweredBy = getHeaderValueCaseInsensitive(responseHeaders, "x-powered-by");
  const corsOrigin = getHeaderValueCaseInsensitive(responseHeaders, "access-control-allow-origin");
  const corsCreds = getHeaderValueCaseInsensitive(responseHeaders, "access-control-allow-credentials");
  const setCookie = getHeaderValueCaseInsensitive(responseHeaders, "set-cookie");
  ...
  if (urlIsHttps && !hsts) {
    pushSecurityFinding(
      ...
      "Missing HSTS",
      "medium",
      "Response does not include Strict-Transport-Security.",
      "Add a strict HSTS policy to enforce HTTPS and prevent downgrade attacks."
    );
  }
  if (!xcto) {
    pushSecurityFinding(
      ...
      "low",
      "Response does not include X-Content-Type-Options.",
  ...
  if (!csp) {
    pushSecurityFinding(
      ...
      "low",
      "Response does not include Content-Security-Policy.",
      "Set a restrictive CSP to reduce injection risk."
    );
  }
  if (!xfo) {
    pushSecurityFinding(
      ...
      "low",
      "Response does not include X-Frame-Options.",
      "Set X-Frame-Options to DENY or SAMEORIGIN."
    );
  }
  if (server) {
    const versionLeak = /\d/.test(server);
    pushSecurityFinding(
      findings,
      "Server Header Disclosure",
      versionLeak ? "medium" : "low",
      `Server header is exposed: ${server}`,
      "Hide or generalize server banner to reduce technology/version fingerprinting."
    );
  }
  if (xPoweredBy) {
    pushSecurityFinding(
      ...
      "medium",
      `X-Powered-By header exposed: ${xPoweredBy}`,
      "Remove framework/version disclosure headers in production."
    );
  }
  if (corsOrigin.trim() === "*" && corsCreds.trim().toLowerCase() === "true") {
    pushSecurityFinding(
      findings,
      "Insecure CORS Configuration",
      "high",
      "Access-Control-Allow-Origin is '*' while credentials are allowed.",
      "Restrict allowed origins and avoid wildcard with credentials."
    );
  }
  if (setCookie) {
    if (!/httponly/i.test(setCookie)) {
      pushSecurityFinding(
        ...
        "medium",
        "Set-Cookie appears without HttpOnly.",
        "Set HttpOnly on authentication/session cookies."
      );
    }
    if (urlIsHttps && !/secure/i.test(setCookie)) {
      pushSecurityFinding(
        findings,
        "Cookie Missing Secure",
        "medium",
        "Set-Cookie appears without Secure on HTTPS endpoint.",
        "Set Secure flag on cookies sent over HTTPS."
      );
    }
    if (!/samesite/i.test(setCookie)) {
      pushSecurityFinding(
        ...
        "low",
        "Set-Cookie appears without SameSite.",
        "Set SameSite=Lax/Strict (or None with Secure when required)."
      );
    }
  }

  const bodySample = responseText.slice(0, 6000);
  if (/stack trace|exception|sqlstate|syntax error|traceback|at\s+\w+\.\w+/i.test(bodySample)) {
    pushSecurityFinding(
      findings,
      "Verbose Error Disclosure",
      responseStatus >= 500 ? "high" : "medium",
      "Response body appears to expose stack/error internals.",
      "Return generic error messages and keep technical details in server logs only."
    );
  }
...
const mergeSecurityFindings = (
  base: SecurityFinding[],
  extra: SecurityFinding[]
): SecurityFinding[] => {
  const merged = new Map<string, SecurityFinding>();
  [...base, ...extra].forEach((finding) => {
    ...
    const next = severityWeight[finding.severity] > severityWeight[existing.severity] ? finding : existing;
    merged.set(key, next);
  });
  return Array.from(merged.values()).sort((a, b) => severityWeight[b.severity] - severityWeight[a.severity]);
};
...
const renderRepeaterFlowStep = (entry: RepeaterEntry) => {
  ...
  if (flow.length === 0) {
    ...
    const parts = buildActiveRepeaterResponseParts(entry);
    setRepeaterResponseContent(parts.raw, parts.headers, parts.body);
    return;
  }
...
const renderRepeaterHeaderRowsFromRows = (rows: RepeaterHeaderRow[]) => {
  const committedHtml = rows
    .map(
    ...
    .join("");
  const addRowHtml = `<tr data-repeater-header-new="true">
    <td><input type="text" data-repeater-header-field="key" value="" placeholder="Header name" /></td>
    <td><input type="text" data-repeater-header-field="value" value="" placeholder="Header value" /></td>
    <td><button type="button" data-repeater-header-action="add" title="Add header">+</button></td>
  </tr>`;
  repeaterHeadersTableBody.innerHTML = `${committedHtml}${addRowHtml}`;
};

const renderRepeaterHeaderRows = (headersText: string) => {
  const rows = getRepeaterHeaderRowsFromText(headersText);
  renderRepeaterHeaderRowsFromRows(rows);
};

const getCommittedRepeaterHeaderRowsFromTable = (): RepeaterHeaderRow[] => {
  const rows = Array.from(repeaterHeadersTableBody.querySelectorAll("tr"));
  const committed: RepeaterHeaderRow[] = [];
  rows.forEach((row) => {
    if (row.getAttribute("data-repeater-header-new") === "true") return;
...
const getPendingRepeaterHeaderRowFromTable = (): RepeaterHeaderRow => {
  const pendingRow = repeaterHeadersTableBody.querySelector<HTMLTableRowElement>('tr[data-repeater-header-new="true"]');
  const keyInput = pendingRow?.querySelector<HTMLInputElement>('input[data-repeater-header-field="key"]');
  const valueInput = pendingRow?.querySelector<HTMLInputElement>('input[data-repeater-header-field="value"]');
  return {
...
const commitPendingRepeaterHeaderRow = (): boolean => {
  const pending = getPendingRepeaterHeaderRowFromTable();
  if (!pending.key) return false;
  const rows = getCommittedRepeaterHeaderRowsFromTable();
  rows.push(pending);
  renderRepeaterHeaderRowsFromRows(rows);
  return true;
};

const removeCommittedRepeaterHeaderRow = (rowEl: HTMLTableRowElement) => {
  rowEl.remove();
  const rows = getCommittedRepeaterHeaderRowsFromTable();
  renderRepeaterHeaderRowsFromRows(rows);
};
...
const resetRepeaterEditorUi = () => {
  activeRepeaterRequestTab = "raw";
  activeRepeaterResponseTab = "raw";
  repeaterMethodInput.value = "GET";
  repeaterUrlInput.value = "";
  repeaterRequestRawInput.value = "";
  repeaterBodyInput.value = "";
  renderRepeaterHeaderRows("");
  setRepeaterResponseContent("", "", "");
  renderRepeaterRequestTab();
  renderRepeaterResponseTab();
};
...
testResultsList.addEventListener("click", (event) => {
  const target = event.target as HTMLElement;
  const endpointEl = target.closest("[data-endpoint-id]") as HTMLElement | null;
  if (!endpointEl) return;
  const endpointId = endpointEl.dataset.endpointId;
  if (!endpointId) return;
  const endpoint = discoveredEndpoints.find((item) => item.id === endpointId);
  const result = endpointTestResults.get(endpointId);
  if (!endpoint || !result) return;
  renderSecurityResultModal(endpoint, result);
});
...
securityResultCloseBtn.addEventListener("click", () => {
  hideSecurityResultModal();
});

securityResultModal.addEventListener("click", (event) => {
  if (event.target === securityResultModal) {
    hideSecurityResultModal();
  }
});
...
const ensuredStartProxyAutomatedTestingBtn = ensureProxyAutomatedTestingButton();
if (ensuredStartProxyAutomatedTestingBtn) {
  ensuredStartProxyAutomatedTestingBtn.addEventListener("click", async () => {
    const endpointIds = getProxyDrivenEndpointIdsForTesting();
    if (endpointIds.length === 0) {
      proxyTrafficStatus.textContent = "No discovered endpoints available for automated testing.";
      return;
    }

    ensuredStartProxyAutomatedTestingBtn.disabled = true;
    proxyTrafficStatus.textContent = `Starting automated testing for ${endpointIds.length} endpoint(s) from proxy traffic...`;
    try {
      await runAutomatedEndpointTesting(endpointIds);
    } finally {
      ensuredStartProxyAutomatedTestingBtn.disabled = false;
    }
  });
}
...
window.addEventListener("keydown", (event) => {
  if (event.key === "Escape" && !securityResultModal.classList.contains("hidden")) {
    hideSecurityResultModal();
  }
});
...
repeaterResponseSection.addEventListener("click", (event) => {
  const target = event.target as HTMLElement;
  if (target.closest("#repeaterResponseRawTabBtn")) {
    activeRepeaterResponseTab = "raw";
    const active = getActiveRepeaterEntry();
    const parts = buildActiveRepeaterResponseParts(active);
    setRepeaterResponseContent(parts.raw, parts.headers, parts.body);
    renderRepeaterResponseTab();
    return;
  }
  if (target.closest("#repeaterResponseHeadersTabBtn")) {
    activeRepeaterResponseTab = "headers";
    const active = getActiveRepeaterEntry();
    const parts = buildActiveRepeaterResponseParts(active);
    setRepeaterResponseContent(parts.raw, parts.headers, parts.body);
    renderRepeaterResponseTab();
  ...
  if (target.closest("#repeaterResponseBodyTabBtn")) {
    activeRepeaterResponseTab = "body";
    const active = getActiveRepeaterEntry();
    const parts = buildActiveRepeaterResponseParts(active);
    setRepeaterResponseContent(parts.raw, parts.headers, parts.body);
    renderRepeaterResponseTab();
...
repeaterHeadersTableBody.addEventListener("input", (event) => {
  const target = event.target as HTMLElement;
...
repeaterHeadersTableBody.addEventListener("click", (event) => {
  const target = event.target as HTMLElement;
  const addBtn = target.closest<HTMLButtonElement>('[data-repeater-header-action="add"]');
  if (addBtn) {
    const added = commitPendingRepeaterHeaderRow();
    if (!added) {
      const pendingKeyInput = repeaterHeadersTableBody.querySelector<HTMLInputElement>(
        'tr[data-repeater-header-new="true"] input[data-repeater-header-field="key"]'
      );
      pendingKeyInput?.focus();
      return;
    }
    syncActiveRepeaterFromInputs();
    return;
  ...
  if (!removeBtn) return;
  const row = removeBtn.closest<HTMLTableRowElement>("tr");
  if (!row) return;
  removeCommittedRepeaterHeaderRow(row);
  syncActiveRepeaterFromInputs();
});
repeaterAddHeaderBtn.addEventListener("click", () => {
  commitPendingRepeaterHeaderRow();
  syncActiveRepeaterFromInputs();
});
...
repeaterSendBtn.addEventListener("click", async () => {
  ...
  if (activeRepeaterRequestTab === "raw") {
    ...
    if (rawError) {
      setRepeaterResponseContent(`Request Error\n\n${rawError}`, "Request Error", rawError);
      return;
    }
  } else {
    syncActiveRepeaterFromInputs();
  ...
  setRepeaterResponseContent("Sending...", "", "");
  active.responseMeta = "";
  active.responseBody = "";
  ...
  try {
    ...
    if (matchedEndpoint) {
      ...
      const analysis = await requestEndpointTestAnalysis(
        matchedEndpoint,
        active.body,
        finalRes.status,
        finalHeaders,
      ...
      endpointTestResults.set(matchedEndpoint.id, {
        ...
        securityOverview: analysis.securityOverview,
        securityFindings: analysis.securityFindings,
        requestHeaders: parseHeaders(active.headers),
        requestBody: active.body,
        responseHeaders: finalHeaders,
        responseBodySnippet: finalText.slice(0, 6000)
      });
      renderTestEndpointsList();
      renderTestResultsList();
    }
...
renderDiscoveredEndpoints();
ensureRepeaterResponseTabMarkup();
renderRepeater();
setupRepeaterResize();
applyProxyColumnWidths();
